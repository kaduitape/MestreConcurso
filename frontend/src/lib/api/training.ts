...
import type { Page, Training, TrainingInput, TrainingMetrics, TrainingProgress } from './types'
...
export const trainingApi = {
  ...
  startProgress: (publicId: string) => api.post<TrainingProgress>(`/training/${publicId}/progress/start`),
  ...
  adminList: (params: { page: number; page_size: number }) =>
    api.get<Page<Training>>(`/admin/training?page=${params.page}&page_size=${params.page_size}`),
  adminTraining: (publicId: string) => api.get<Training>(`/admin/training/${publicId}`),
  metrics: (publicId: string) => api.get<TrainingMetrics>(`/admin/training/${publicId}/metrics`),
  ...
  saveScript: (publicId: string, input: { title: string; script: Record<string, unknown> }) =>
