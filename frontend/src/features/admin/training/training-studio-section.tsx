...
function scriptOf(lesson: Training): TrainingScript {
  // Os padrões precisam vir DEPOIS do spread: antes, uma aula com `scenes`
  // ausente sobrescrevia o `[]` e a tela iterava sobre `undefined`.
  const script = lesson.script ?? ({} as TrainingScript)
  return { ...script, objectives: script.objectives ?? [], scenes: script.scenes ?? [] }
}
...
export function TrainingStudioSection() {
  ...
  const create = useMutation({
    ...
    onError: (error: unknown) =>
      toast.error(
        error instanceof ApiError ? error.message : 'Não foi possível criar o rascunho.',
      ),
  })
  ...
  return (
    <div className="space-y-6">
      <Alert tone="info" title="Estúdio de Treinamento">
        A IA gera um roteiro em cenas editáveis. Nada é publicado automaticamente: revise
        diálogo, destaques e pergunta antes de liberar para os candidatos.
      </Alert>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Sparkles /> Criar treinamento
            </CardTitle>
            <CardDescription>
              Transforme um tema de concurso em uma missão de aprendizagem.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form
            ...
            >
              ...
              <Field label="Personagem" htmlFor="training-character">
                <Input
                  id="training-character"
                  required
                  value={form.character_name}
                  onChange={(event) => setForm({ ...form, character_name: event.target.value })}
                />
              </Field>
              <Field label="Banca (opcional)" htmlFor="training-board">
                <Input
                  ...
                  onChange={(event) => setForm({ ...form, board_name: event.target.value })}
                  placeholder="FGV"
                />
              </Field>
              <Field label="Nível" htmlFor="training-level">
                <select
                  ...
                  value={form.level}
                  onChange={(event) =>
                    setForm({ ...form, level: event.target.value as TrainingInput['level'] })
                  }
                >
              ...
              <Field label="Estilo" htmlFor="training-style">
                <select
                  ...
                  value={form.style}
                  onChange={(event) =>
                    setForm({ ...form, style: event.target.value as TrainingInput['style'] })
                  }
                >
                  <option value="AULA">Aula normal</option>
                  <option value="HISTORIA">História fantástica</option>
                  <option value="MISSAO">Missão</option>
                  <option value="BATALHA">Batalha</option>
                  <option value="INVESTIGACAO">Investigação</option>
                  <option value="MILITAR">Treinamento militar</option>
              ...
              <Field label="Duração" htmlFor="training-duration">
                <select
                  ...
                  onChange={(event) =>
                    setForm({ ...form, target_duration_minutes: Number(event.target.value) })
                  }
                >
              ...
              <Field
                ...
                htmlFor="training-prompt"
                hint="Ex.: explique de modo simples e destaque pegadinhas da FGV."
              >
                <Textarea
                  id="training-prompt"
                  value={form.additional_prompt}
                  onChange={(event) =>
                    setForm({ ...form, additional_prompt: event.target.value })
                  }
                />
              </Field>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            {lessons.data?.items.length === 0 && (
              <p className="text-sm text-muted">Nenhum treinamento criado ainda.</p>
            )}
            {lessons.data?.items.map((lesson) => (
              <button
                type="button"
                key={lesson.public_id}
                onClick={() => setSelectedId(lesson.public_id)}
                className="w-full rounded-md border border-border p-3 text-left transition hover:bg-surface-muted"
              >
                ...
                <p className="mt-1 text-xs text-muted">
                  {lesson.subject} · {lesson.character_name}
                </p>
              </button>
            ))}
          </CardContent>
      ...
      {selected.data && (
        <TrainingEditor
          ...
          onGenerate={() => generate.mutate(selected.data!.public_id)}
          onSaved={invalidate}
        />
      )}
    </div>
...
function TrainingEditor({
  ...
  useEffect(() => {
    ...
    setScript(scriptOf(lesson))
  }, [lesson])
  const scenes = script.scenes ?? []
  const pipeline = useMemo(() => STEPS, [])
  const updateScene = (index: number, patch: Partial<TrainingScene>) =>
    setScript((current) => ({
      ...current,
  ...
  const save = useMutation({
    mutationFn: () => trainingApi.saveScript(lesson.public_id, { title, script }),
    onSuccess: (result) => {
      queryClient.setQueryData(queryKeys.adminTrainingLesson(result.public_id), result)
      onSaved()
      toast.success('Cenas salvas.')
    },
    onError: (error: unknown) =>
      toast.error(
        error instanceof ApiError ? error.message : 'Não foi possível salvar as cenas.',
      ),
  })
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle className="flex items-center gap-2">
              <FilePenLine /> Editor do treinamento
            </CardTitle>
            <CardDescription>
              {lesson.status === 'DRAFT'
                ? 'Gere o roteiro para abrir as cenas.'
                : `${scenes.length} cena(s) editáveis`}
            </CardDescription>
          </div>
          <Badge variant={lesson.status === 'PUBLISHED' ? 'success' : 'neutral'}>
            {statusLabel[lesson.status]}
      ...
      <CardContent className="space-y-5">
        {lesson.generation_error && (
          <Alert tone="danger" title="A geração falhou">
            {lesson.generation_error}
          </Alert>
        )}
        {generating && (
          <div className="rounded-md border border-primary/30 bg-primary/5 p-4">
            <p className="font-medium">Gerando treinamento com IA…</p>
            <ol className="mt-3 grid gap-2 text-sm text-muted sm:grid-cols-2">
              {pipeline.map((step) => (
                <li key={step} className="flex items-center gap-2">
                  <Check className="size-4 animate-pulse text-primary" />
                  {step}
                </li>
              ))}
            </ol>
        ...
        <div className="flex flex-wrap gap-2">
          <Button loading={generating} onClick={onGenerate}>
            <Sparkles /> {scenes.length ? 'Gerar novo roteiro' : 'Gerar treinamento com IA'}
          </Button>
          {scenes.length > 0 && (
            <>
              ...
              <Button variant="outline" asChild>
                <Link to={`/dia-de-treinamento/${lesson.public_id}`} target="_blank">
                  <CirclePlay /> Visualizar
                </Link>
              </Button>
              <Button
              ...
              >
                <BookOpenText /> {lesson.status === 'PUBLISHED' ? 'Publicado' : 'Publicar'}
              </Button>
            </>
          )}
        </div>
        {lesson.status === 'PUBLISHED' && metrics.data && (
          <div className="grid gap-3 rounded-md border border-border bg-surface-muted/40 p-4 sm:grid-cols-4">
            <div>
              <p className="text-xs text-muted">Inícios</p>
              <strong>{metrics.data.starts}</strong>
            </div>
            <div>
              <p className="text-xs text-muted">Conclusões</p>
              <strong>{metrics.data.completions}</strong>
            </div>
            <div>
              <p className="text-xs text-muted">Taxa de conclusão</p>
              <strong>{Math.round(metrics.data.completion_rate * 100)}%</strong>
            </div>
            <div>
              <p className="text-xs text-muted">Foco médio</p>
              <strong>{Math.round(metrics.data.average_focus_seconds / 60)} min</strong>
            </div>
          </div>
        )}
        {scenes.length > 0 && (
          <>
            <Field label="Título" htmlFor="training-title">
              <Input
                id="training-title"
                value={title}
            ...
            <div className="space-y-4">
              {scenes.map((scene, index) => (
                <div key={scene.id ?? index} className="rounded-md border border-border p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <strong>Cena {index + 1}</strong>
                    <Badge variant="neutral">{scene.type}</Badge>
                  </div>
                  <div className="grid gap-3 lg:grid-cols-2">
                    <Field label="Fala / diálogo" htmlFor={`dialogue-${index}`}>
                      <Textarea
                        id={`dialogue-${index}`}
                    ...
                    <Field label="Texto na tela" htmlFor={`screen-text-${index}`}>
                      <Textarea
                        id={`screen-text-${index}`}
                        value={scene.screen_text ?? ''}
                        onChange={(event) =>
                          updateScene(index, { screen_text: event.target.value })
                        }
                      />
                    </Field>
                    <Field
                    ...
                    <Field label="Duração (segundos)" htmlFor={`duration-${index}`}>
                      <Input
                        id={`duration-${index}`}
                        type="number"
                        min="3"
                        ...
                        onChange={(event) =>
                          updateScene(index, { duration: Number(event.target.value) || 12 })
                        }
                      />
                    </Field>
                  </div>
