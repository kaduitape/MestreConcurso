...
@training_router.get("/{public_id}", response_model=TrainingRead, summary="Abrir treinamento publicado")
async def get_published(public_id: str, _: CurrentUser, db: DbSession) -> TrainingRead:
    return TrainingRead.model_validate(await TrainingService(db).get(public_id, published_only=True))


@training_router.post("/{public_id}/progress/start", response_model=TrainingProgressRead, summary="Iniciar ou retomar missão")
...
@training_router.put("/{public_id}/progress", response_model=TrainingProgressRead, summary="Salvar progresso da missão")
...
@training_router.post("/{public_id}/complete", response_model=TrainingProgressRead, summary="Concluir missão e creditar XP")
async def complete_progress(public_id: str, user: CurrentUser, db: DbSession) -> TrainingProgressRead:
    service = TrainingService(db)
    lesson = await service.get(public_id, published_only=True)
...
@admin_router.post("", response_model=TrainingRead, status_code=status.HTTP_201_CREATED, summary="Criar rascunho")
async def create_training(payload: TrainingCreate, actor: TrainingWriter, db: DbSession, ctx: RequestCtx) -> TrainingRead:
    lesson = await TrainingService(db).create(actor, payload.model_dump(), ctx)
    return TrainingRead.model_validate(lesson)


@admin_router.get("/{public_id}", response_model=TrainingRead, summary="Abrir treinamento no editor")
...
@admin_router.get("/{public_id}/metrics", response_model=TrainingMetricsRead, summary="Métricas reais do treinamento")
...
@admin_router.post("/{public_id}/generate", response_model=TrainingRead, summary="Gerar roteiro com IA")
async def generate_training(public_id: str, actor: TrainingWriter, db: DbSession, ctx: RequestCtx) -> TrainingRead:
    service = TrainingService(db)
    lesson = await service.generate(await service.get(public_id), actor, ctx)
    return TrainingRead.model_validate(lesson)


@admin_router.put("/{public_id}/script", response_model=TrainingRead, summary="Salvar edição das cenas")
async def update_script(
    public_id: str, payload: TrainingScriptUpdate, actor: TrainingWriter, db: DbSession, ctx: RequestCtx
) -> TrainingRead:
    service = TrainingService(db)
    lesson = await service.update_script(
        await service.get(public_id), title=payload.title, script=payload.script, actor=actor, context=ctx
    )
    return TrainingRead.model_validate(lesson)


@admin_router.post("/{public_id}/publish", response_model=TrainingRead, summary="Publicar treinamento")
async def publish_training(public_id: str, actor: TrainingWriter, db: DbSession, ctx: RequestCtx) -> TrainingRead:
    service = TrainingService(db)
